...
def admin_dashboard():
    ...
    with c2:
        active_count = len(users[users['status'] == 'active']) if not users.empty and 'status' in users.columns else 0
        render_kpi_card("Ativos", active_count, icon="✅", is_currency=False)
    with c3:
        pending_count = len(users[users['status'] == 'pending']) if not users.empty and 'status' in users.columns else 0
