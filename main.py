...
def main():
    ...
    if user['role'] == 'admin':
        if selected == "🏠 Início":
            # Just show a welcome or redirect to dashboard
            admin_dashboard()
        elif selected == "🛡️ Painel Admin":
            admin_dashboard()
...
def admin_dashboard():
            ...
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div style="margin-left: auto;">
                    <a href="/" target="_self" style="text-decoration: none;">
                        <button style="background: rgba(239, 68, 68, 0.2); border: 1px solid #ef4444; color: white; padding: 8px 16px; border-radius: 8px; cursor: pointer;">Sair do App</button>
                    </a>
                </div>
            </div>
        </div>
    ...
    with tab_overview:
        ...
        st.markdown("---")
        st.subheader("🛡️ Segurança do Clã")
        st.info("A privacidade dos membros é garantida conforme a LGPD. O administrador tem acesso apenas à gestão de contas e prazos.")
