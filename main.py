...
# Page Config
st.set_page_config(
    page_title="FinanFlow - Gestão Inteligente",
    page_icon="🛡️",
    layout="wide",
...
def local_css():
    ...
    <style>
        ...
        * {
        ...
        /* header {visibility: hidden;}  <- DO NOT HIDE HEADER, it contains the sidebar toggle */
...
def render_kpi_card(label, value, delta=None, icon="💰", sparkline_data=None, show_delta=True, is_currency=True, color_theme="primary"):
    ...
    # Minified HTML without any newlines or indentation to prevent markdown parsing errors
    card_html = (
        ...
        f'<span style="font-size: 1.2rem;">{icon}</span><span style="font-weight: 500;">{label}</span></div>'
...
def tab_dashboard(user):
    ...
    with i_col2:
        if not expenses.empty:
            ...
            st.markdown(f"""<div class="insight-card" style="border-left-color: #64748b;"><b>Perfil de Gasto</b><br>Nenhuma despesa para análise.</div>""", unsafe_allow_html=True)
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                # Stable models based on API model list
                available_models = [
                    'gemini-1.5-flash',
                    'gemini-flash-latest', 
                    'gemini-pro-latest',
    ...
    with st.sidebar:
        ...
        st.markdown(f"""
            <div style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); padding: 25px; border-radius: 16px; margin-bottom: 1.5rem; text-align: center; border: 1px solid rgba(16, 185, 129, 0.2);">
                ...
                <div style="color: white; font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 1.4rem; letter-spacing: -0.5px;">FinanFlow</div>
                <div style="color: #10b981; font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 2px; margin-top: 4px;">Gestão Inteligente</div>
            </div>
        """, unsafe_allow_html=True)
        ...
        # Navigation
        if user['role'] == 'admin':
            menu_options = ["🏠 Início", "🛡️ Painel Admin"]
...
def admin_dashboard():
    ...
    st.markdown("""
        <div style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); padding: 2.5rem; border-radius: 20px; color: white; margin-bottom: 2rem; box-shadow: var(--shadow-lg); border-bottom: 4px solid #10b981;">
            <div style="display: flex; align-items: center; gap: 20px;">
                <div style="background: #10b981; padding: 15px; border-radius: 12px; box-shadow: 0 0 20px rgba(16,185,129,0.3);">
                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M12 8v4"/><path d="M12 16h.01"/></svg>
                </div>
                <div>
                    <h1 style="margin: 0; color: white; font-family: 'Outfit', sans-serif; font-size: 2.2rem; letter-spacing: -1px;">FinanFlow</h1>
                    <p style="opacity: 0.8; margin: 5px 0 0 0; font-size: 1.1rem;">Gestão Financeira Inteligente</p>
                </div>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    # 3. Área de Gestão
    st.markdown("""
        <div style="padding: 10px 0; border-bottom: 2px solid #e2e8f0; margin-bottom: 1.5rem;">
            <h3 style="margin: 0; color: #0f172a;">👤 Gestão de Usuários</h3>
        </div>
    """, unsafe_allow_html=True)
    ...
    with cf2:
        status_filter = st.selectbox("📂 Filtrar Status", ["Todos", "active", "pending", "blocked"], index=0)
    ...
    for index, row in filtered_users.iterrows():
        # Auto-block check if expired
        expiry = row.get('expiry_date')
        from datetime import datetime
        if expiry and not pd.isna(expiry) and datetime.now() > expiry and row['status'] != 'blocked':
        ...
        with st.container():
            ...
            expiry_str = f"Expira em: {row['expiry_date'].strftime('%d/%m/%Y')}" if row.get('expiry_date') and not pd.isna(row['expiry_date']) else "Acesso Vitalício"
                ...
                st.markdown(f"""
                    <div style="padding: 15px; border-radius: 12px; border: 1px solid #e2e8f0; background: white; margin-bottom: 10px;">
                        ...
                        <div style="font-size: 0.8rem; color: #64748b; margin-top: 5px; margin-left: 20px;">
                            📅 {expiry_str} • IDs: {row['id']} • Criado em: {row['created_at']}
                        </div>
                    </div>
                """, unsafe_allow_html=True)
            ...
            with c_actions:
                ...
                with st.popover("📅 Prazo", use_container_width=True):
                    dias = st.number_input("Dias de Acesso", min_value=1, max_value=365, value=30, key=f"days_{row['id']}")
                    if st.button("Confirmar", key=f"set_exp_{row['id']}"):
    ...
    # 4. Rodapé e Segurança
    st.markdown("<br><br>", unsafe_allow_html=True)
    footer_cols = st.columns([2, 1])
    with footer_cols[0]:
        st.markdown("""
            <div style="background: #f8fafc; padding: 20px; border-radius: 12px; border-left: 5px solid #0f172a;">
                <h4 style="margin: 0; color: #0f172a;">🛡️ Segurança e Privacidade</h4>
                <p style="margin: 5px 0 0 0; color: #64748b; font-size: 0.9rem;">A privacidade dos usuários é garantida conforme a LGPD. O administrador não visualiza dados financeiros privados.</p>
            </div>
        """, unsafe_allow_html=True)
    with footer_cols[1]:
        st.markdown(f"""
            <div style="text-align: right; color: #94a3b8; font-size: 0.8rem; margin-top: 10px;">
                🟢 Banco Neon Sincronizado<br>
