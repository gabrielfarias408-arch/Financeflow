...
def tab_registros(user):
    ...
    if not df.empty:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"Clan Financeiro_relatorio_{datetime.date.today()}.csv",
            mime="text/csv",
        )
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                # Model Fallback Logic - Using current stable models
                # gemini-1.5-flash is optimized for speed and free tier
                available_models = ['gemini-1.5-flash', 'gemini-1.5-flash-8b', 'gemini-1.5-pro']
...
def admin_dashboard():
    ...
    st.markdown("""
        <div style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); padding: 2.5rem; border-radius: 20px; color: white; margin-bottom: 2rem; box-shadow: var(--shadow-lg); border-bottom: 4px solid #10b981;">
            <div style="display: flex; align-items: center; gap: 20px;">
                ...
                <div>
                    <h1 style="margin: 0; color: white; font-family: 'Outfit', sans-serif; font-size: 2.2rem; letter-spacing: -1px;">Clan Financeiro</h1>
                    <p style="opacity: 0.8; margin: 5px 0 0 0; font-size: 1.1rem;">Gestão Inteligente e Controle de Elite</p>
                </div>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"Exibindo **{len(filtered_users)}** membros")
    ...
    with footer_cols[0]:
        st.markdown("""
            <div style="background: #f8fafc; padding: 20px; border-radius: 12px; border-left: 5px solid #0f172a;">
                <h4 style="margin: 0; color: #0f172a;">🛡️ Segurança do Clã</h4>
                <p style="margin: 5px 0 0 0; color: #64748b; font-size: 0.9rem;">A privacidade dos membros é garantida conforme a LGPD. O administrador não visualiza dados financeiros privados.</p>
            </div>
        """, unsafe_allow_html=True)
    with footer_cols[1]:
        st.markdown(f"""
