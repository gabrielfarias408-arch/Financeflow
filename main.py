...
def render_kpi_card(label, value, delta=None, icon="💰", sparkline_data=None, show_delta=True):
    ...
    html = f"""<div class="metric-card"><div style="display: flex; flex-direction: column; height: 100%;"><div class="metric-label" style="display: flex; align-items: center; gap: 8px;"><span style="font-size: 1.2rem;">{icon}</span> <span>{label}</span></div><div class="metric-value">R$ {value:,.2f}</div>{delta_html}</div></div>"""
