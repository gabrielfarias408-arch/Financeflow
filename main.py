...
if "DATABASE_URL" not in st.secrets and "general" not in st.secrets:
    st.sidebar.warning("⚠️ Configuração do Banco de Dados não encontrada nos Secrets.")
elif not (st.secrets.get("DATABASE_URL") or st.secrets.get("general", {}).get("DATABASE_URL")):
    st.sidebar.warning("⚠️ DATABASE_URL está vazia nos Secrets.")
...
def admin_dashboard():
    ...
    if users.empty:
        st.warning("📊 Nenhum usuário encontrado no banco de dados. Verifique a conexão com o Neon.")
        return
