...
def render_kpi_card(label, value, delta=None, icon="💰", sparkline_data=None, show_delta=True, is_currency=True, color_theme="primary"):
    ...
    html = f"""
    <div class="metric-card" style="border-left: 4px solid {card_color}; transition: all 0.3s ease;">
        <div style="display: flex; flex-direction: column; height: 100%;">
            <div class="metric-label" style="display: flex; align-items: center; gap: 8px;">
                <span style="font-size: 1.2rem;">{icon}</span> 
                <span style="font-weight: 500;">{label}</span>
            </div>
            <div class="metric-value" style="color: {card_color if not is_currency else '#0f172a'};">{value_display}</div>
            {delta_html}
        </div>
    </div>
    """
...
def admin_dashboard():
    ...
    with tab_overview:
        ...
        # Simple stats/chart could go here
        st.info("O painel de atividades está em sincronia com o banco de dados Neon.")
                ...
                with c_actions:
                    ...
                    with btn_cols[0]:
                        if row['status'] == 'pending':
                            if st.button("✅ Aprovar", key=f"app_{row['id']}", use_container_width=True):
                                db.run_query("UPDATE users SET status = 'active' WHERE id = %s", (row['id'],))
                                st.success("Aprovado!")
                                st.rerun()
                        elif row['status'] == 'active':
                            if st.button("🚫 Bloquear", key=f"blk_{row['id']}", use_container_width=True):
                                db.run_query("UPDATE users SET status = 'blocked' WHERE id = %s", (row['id'],))
                                st.rerun()
                        else:
                            if st.button("🔓 Ativar", key=f"unblk_{row['id']}", use_container_width=True):
                                db.run_query("UPDATE users SET status = 'active' WHERE id = %s", (row['id'],))
                                st.rerun()
                    
                    with btn_cols[1]:
                        if row['role'] != 'admin':
                            if st.button("⭐ Admin", key=f"promo_{row['id']}", use_container_width=True, help="Promover a Administrador"):
                                db.run_query("UPDATE users SET role = 'admin' WHERE id = %s", (row['id'],))
                                st.success("Promovido!")
                                st.rerun()
                    
                    with btn_cols[2]:
                        # Optional: Reset password or Delete
                        if st.button("🗑️", key=f"del_{row['id']}", help="Excluir Usuário (CUIDADO)", use_container_width=True):
                            # Small safeguard: don't delete self
                            if row['email'] != st.session_state.user['email']:
                                db.run_query("DELETE FROM users WHERE id = %s", (row['id'],))
                                st.rerun()
                            else:
                                st.error("Não pode se deletar!")
