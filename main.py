    ...
    <style>
        ...
        * {
        ...
        .metric-card {
            ...
            height: 100%;
            min-width: 0; /* Fix flex truncation */
        }
        ...
        .delta-up { background-color: #ecfdf5; color: var(--success); }
        .delta-down { background-color: #fef2f2; color: var(--danger); }
...
# --- KPI Components ---
def render_kpi_card(label, value, delta=None, icon="💰", sparkline_data=None):
    delta_html = ""
    # Only show delta if it is not None AND not zero (to hide "0.0% vs ant.")
    if delta is not None and abs(delta) > 0.001:
        delta_class = "delta-up" if delta >= 0 else "delta-down"
        delta_icon = "↑" if delta >= 0 else "↓"
        delta_html = f"""
            <div class="metric-delta {delta_class}">
                {delta_icon} {abs(delta):.1f}% <span style="opacity: 0.7; font-weight: 400; font-size: 0.7rem; margin-left: 2px;">vs ant.</span>
            </div>
        """
    ...
    <div class="metric-card">
        <div style="display: flex; flex-direction: column; height: 100%;">
            <div class="metric-label" style="display: flex; align-items: center; gap: 8px;">
                <span style="font-size: 1.2rem;">{icon}</span> <span>{label}</span>
            </div>
...
def tab_registros(user):
    ...
    with c1:
        render_kpi_card("Ganhos", income, icon="💰")
    with c2:
        render_kpi_card("Gastos", expense, icon="📉") # Delta removed as requested
    with c3:
        render_kpi_card("Investimentos", investment, icon="🏦")
    with c4:
        render_kpi_card("Saldo", balance, icon="⚖️")
