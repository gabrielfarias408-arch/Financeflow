...
# --- KPI Components ---
def render_kpi_card(label, value, delta=None, icon="💰", sparkline_data=None, show_delta=True):
    delta_html = ""
    ...
    html = f"""<div class="metric-card"><div style="display: flex; flex-direction: column; height: 100%;"><div class="metric-label" style="display: flex; align-items: center; gap: 8px;"><span style="font-size: 1.2rem;">{icon}</span> <span>{label}</span></div><div class="metric-value"><span style="font-size: 1rem; opacity: 0.8; margin-right: 4px;">R$</span>{value:,.2f}</div>{delta_html}</div></div>"""
...
def tab_investimentos(user):
    ...
    # Get portfolio value UP TO the selected date
    total_patrimonio = db.get_total_portfolio_value(user['id'], as_of_date=as_of_date)
    ...
    # For the table and category cards, we ALSO use as_of_date
    portfolio_df = db.get_portfolio_summary(user['id'], as_of_date=as_of_date)
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                available_models = ['gemini-1.5-flash', 'gemini-1.5-flash-latest', 'gemini-pro-latest']
                ...
                with st.status("🤖 FinanBot está analisando seus dados...", expanded=True) as status:
                    for model_name in available_models:
                        try:
                            # Silence model connection logs to avoid showing quota errors to user
...
def main():
    ...
    # Sidebar
    with st.sidebar:
        # Premium Logo Placeholder (using a modern SVG-like style via HTML/CSS)
        st.markdown("""
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 2rem; padding: 10px; background: linear-gradient(135deg, #3b82f6 0%, #2dd4bf 100%); border-radius: 12px; color: white;">
                ...
                <div>
                    <div style="font-weight: 700; font-size: 1.2rem; line-height: 1.2;">FinanFlow</div>
                    <div style="font-size: 0.7rem; opacity: 0.8; text-transform: uppercase; letter-spacing: 0.05em;">Intelligent Finance</div>
                </div>
            </div>
        """, unsafe_allow_html=True)
...
def admin_dashboard():
    ...
    with c1:
        render_kpi_card("Total Usuários", len(users), icon="👥")
    with c2:
        render_kpi_card("Ativos", len(users[users['status'] == 'active']), icon="✅")
    with c3:
        render_kpi_card("Pendentes", len(users[users['status'] == 'pending']), icon="⏳")
