...
if "DATABASE_URL" not in st.secrets and "general" not in st.secrets:
    st.warning("⚠️ Erro Crítico: Configuração do Banco de Dados não encontrada nos Secrets do Streamlit Cloud.")
    st.info("Para corrigir: Vá em 'Manage App' -> 'Settings' -> 'Secrets' e cole sua DATABASE_URL.")
elif not (st.secrets.get("DATABASE_URL") or st.secrets.get("general", {}).get("DATABASE_URL")):
    st.error("⚠️ DATABASE_URL está presente mas está VAZIA.")

# Deep Debug for Connection Issues (Visible only to developers/admins)
with st.sidebar.expander("🛠️ Diagnóstico de Conexão"):
    st.write(f"Sessão: {st.session_state.get('db_initialized', False)}")
    keys = list(st.secrets.keys())
    st.write(f"Chaves no Secret: {keys}")
    if "general" in st.secrets:
        st.write(f"Chaves em [general]: {list(st.secrets['general'].keys())}")
...
def local_css():
    ...
    <style>
        ...
        :root {
            ...
            --danger: #ef4444;
            --warning: #f59e0b;
            --background: #f1f5f9; /* Slightly darker for better contrast with cards */
            --surface: #ffffff;
            --text-main: #0f172a; /* Deeper navy for premium feel */
            --text-muted: #64748b;
            --border: #e2e8f0;
        ...
        * {
        ...
        /* Force Menu Visibility - Legible text against light background */
...
# --- KPI Components ---
def render_kpi_card(label, value, delta=None, icon="💰", sparkline_data=None, show_delta=True, is_currency=True):
    delta_html = ""
    ...
    html = f"""<div class="metric-card"><div style="display: flex; flex-direction: column; height: 100%;"><div class="metric-label" style="display: flex; align-items: center; gap: 8px;"><span style="font-size: 1.2rem;">{icon}</span> <span>{label}</span></div><div class="metric-value">{value_display}</div>{delta_html}</div></div>"""
...
def main():
    ...
    with st.sidebar:
        # Premium Flow Logo
        st.markdown("""
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 1.5rem; padding: 10px; border-radius: 12px;">
                <div style="background: linear-gradient(135deg, #3b82f6 0%, #10b981 100%); padding: 10px; border-radius: 12px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 15px rgba(59, 130, 246, 0.2);">
                ...
                <div>
                    <div style="font-weight: 700; font-size: 1.3rem; line-height: 1.2; color: #0f172a; font-family: 'Outfit', sans-serif;">FinanFlow</div>
                    <div style="font-size: 0.75rem; color: #3b82f6; font-weight: 600; text-transform: uppercase; letter-spacing: 0.1em;">Smart Intel</div>
                </div>
            </div>
            <hr style="margin: 0 0 1.5rem 0; border: none; border-top: 1px solid #e2e8f0; opacity: 0.5;">
        """, unsafe_allow_html=True)
...
def admin_dashboard():
    ...
    if users.empty:
        ...
        with st.expander("🛠️ Ver Detalhes do Erro de Conexão"):
            st.write(f"Chaves lidas pelo app: {list(st.secrets.keys())}")
            if st.button("🔌 Tentar Conectar Manualmente"):
                if db.get_connection():
                    st.success("Conexão estabelecida com sucesso! Recarregue a página.")
                else:
                    st.error("Falha ao abrir conexão. Verifique se a URL no Neon está correta e se o IP do Streamlit não está bloqueado.")
        return

    c1, c2, c3 = st.columns(3)
    with c1:
        render_kpi_card("Total Usuários", len(users), icon="👥", is_currency=False)
    with c2:
        active_count = len(users[users['status'] == 'active']) if 'status' in users.columns else 0
        render_kpi_card("Ativos", active_count, icon="✅", is_currency=False)
    with c3:
        pending_count = len(users[users['status'] == 'pending']) if 'status' in users.columns else 0
        render_kpi_card("Pendentes", pending_count, icon="⏳", is_currency=False)
